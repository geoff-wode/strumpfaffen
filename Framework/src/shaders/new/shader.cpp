...
static void GetUniformBlock(struct ShaderImpl* shader, GLuint blockIndex);
static void GetUniforms(struct ShaderImpl* shader);
...
static void GetUniforms(ShaderImpl* shader)
{
...
static void GetUniformBlock(ShaderImpl* shader, GLuint blockIndex)
{
	static GLuint nextBinding = 0;
	static std::map<std::string,UniformBlock*> bufferBindings;
	...
	const std::map<std::string,UniformBlock*>::iterator i = bufferBindings.find(blockName.data());
	if (bufferBindings.end() != i)
	{
		...
		block.buffer = Buffers::CreateShaderUniformBuffer(block.bindingPoint, blockSize);
